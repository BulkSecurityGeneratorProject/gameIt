            ...
            controller: ('CartController', ['$rootScope', '$scope', 'ngCart', 'fulfilmentProvider', function ($rootScope, $scope, ngCart, fulfilmentProvider) {
                ...
                $scope.checkout = function () {
                    ...
                    fulfilmentProvider.checkout()
                        .then(function () {
                            $rootScope.$broadcast('ngCart:checkout_succeeded');
                        }, function () {
...
angular.module('ngCart.fulfilment', [])
    ...
    .service('ngCart.fulfilment.http', ['$http', 'ngCart', '$uibModal', '$rootScope', '$q',
        function ($http, ngCart, $uibModal, $rootScope, $q) {
        ...
        this.checkout = function () {
            ...
            for (var i = 0; i < items.length; i++) {
                for (var j = 0; j < items[i].getQuantity(); j++) {
                    games.push(items[i].getData());
                }
            }
            var open = function (size) {
                var modalInstance = $uibModal.open({
                    ...
                    templateUrl: 'views/credit-card-modal.html',
                ...
                modalInstance.result.then(function (games) {
                    q.resolve(games);
                    ngCart.empty();

                }, function (error,games) {
                    console.log('modalClosed');
                    q.reject(error,games);
                });
            };
            open('md');
            return q.promise;
